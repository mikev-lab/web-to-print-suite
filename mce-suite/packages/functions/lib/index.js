...
exports.getDynamicPrice = exports.assemblePrintPDF = void 0;
...
exports.assemblePrintPDF = (0, https_1.onCall)(async (request) => {
    ...
    try {
        ...
        if (!specs || !spineWidthInches || !fileUploads) {
            throw new https_1.HttpsError('failed-precondition', 'Order document is missing required fields.');
        }
        ...
        const coverPdfDoc = await pdf_lib_1.PDFDocument.create();
        const page = coverPdfDoc.addPage([totalWidthInches * 72, totalHeightInches * 72]);
        const embedImage = async (fileBuffer, fileType) => {
            if (fileType.startsWith('image/')) {
                ...
                const externalPdf = await pdf_lib_1.PDFDocument.load(fileBuffer);
                const [embeddedPage] = await coverPdfDoc.embedPdf(externalPdf, [0]);
                return embeddedPage;
            }
            throw new https_1.HttpsError('invalid-argument', `Unsupported file type: ${fileType}`);
        };
        const drawCoverPart = async (fileBuffer, path, x, y, width, height, transform) => {
            ...
            if (!embeddedContent) {
                throw new https_1.HttpsError('internal', 'Failed to embed content.');
            }
            ...
            if (embeddedContent instanceof pdf_lib_1.PDFImage) {
                page.drawImage(embeddedContent, { x: imgX, y: imgY, width: drawWidth, height: drawHeight });
            }
            else if (embeddedContent instanceof pdf_lib_1.PDFEmbeddedPage) {
        ...
        throw new https_1.HttpsError('internal', 'PDF assembly failed. Please check your files and try again.');
    }
});
exports.getDynamicPrice = (0, https_1.onCall)(async (request) => {
    const { data: specs } = request;
    if (!specs) {
        throw new https_1.HttpsError('invalid-argument', 'The function must be called with one argument "specs".');
    }
    const businessRulesDoc = await db.collection('config').doc('business_rules').get();
    if (!businessRulesDoc.exists) {
        throw new https_1.HttpsError('not-found', 'Business rules not found.');
    }
    const businessRules = businessRulesDoc.data();
    const { laborRate = businessRules.defaultLaborRate, markupPercent = businessRules.defaultMarkupPercent, spoilagePercent = businessRules.defaultSpoilagePercent, } = businessRules;
    ...
    const calculateCosts = async (details) => {
        const { quantity, finishedWidth, finishedHeight, bwPages, bwPaperSku, colorPages, colorPaperSku, hasCover, coverPaperSku, coverPrintColor, coverPrintsOnBothSides, laminationType, bindingMethod, } = details;
        ...
        if (bindingMethod === 'saddleStitch' && totalInteriorPages > 0 && totalInteriorPages % 4 !== 0) {
            throw new https_1.HttpsError('invalid-argument', 'Saddle stitch requires the total interior page count to be a multiple of 4.');
        }
        ...
        if (bwPaper && bwImposition === 0 && bwPages > 0)
            throw new https_1.HttpsError('invalid-argument', 'Finished size does not fit on the B/W interior paper.');
        if (colorPaper && colorImposition === 0 && colorPages > 0)
            throw new https_1.HttpsError('invalid-argument', 'Finished size does not fit on the Color interior paper.');
        if (hasCover && coverPaper && coverImposition === 0)
            throw new https_1.HttpsError('invalid-argument', 'Full cover spread (including spine) does not fit on the selected cover paper.');
        ...
        if (hasCover) {
            ...
            const coverClickRate = coverPrintColor === types_1.PrintColor.COLOR ? businessRules.COLOR_CLICK_COST : businessRules.BW_CLICK_COST;
